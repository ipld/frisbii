...
func (cm CarMetadata) Serialize() ([]byte, error) {
	// TODO: do the same checks we do on Deserialize()
	return BindnodeRegistry.TypeToBytes(&cm, dagjson.Encode)
}

func (cm *CarMetadata) Deserialize(byts []byte) error {
	cmIface, err := BindnodeRegistry.TypeFromBytes(byts, &CarMetadata{}, dagjson.Decode)
	if err != nil {
