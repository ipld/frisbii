...
func TestCarMetadataRoundtrip(t *testing.T) {
	...
	orig := metadata.CarMetadata{
		Metadata: &metadata.Metadata{
			...
			Properties: &metadata.CarProperties{
				CarBytes:          202020,
				DataBytes:         101010,
				BlockCount:        303,
	...
	byts, err := orig.Serialize()
	require.NoError(t, err)

	t.Log("metadata dag-json:", string(byts))

	var roundtrip metadata.CarMetadata
	err = roundtrip.Deserialize(byts)
	require.NoError(t, err)
...
func TestBadMetadata(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			var roundtrip metadata.CarMetadata
			require.ErrorContains(t, roundtrip.Deserialize([]byte(tc.byts)), tc.err)
		})
	}
}
